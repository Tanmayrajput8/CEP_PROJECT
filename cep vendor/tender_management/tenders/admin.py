...
class VendorDocumentInline(nested_admin.NestedTabularInline):  
...
# Vendor Inline ( includes Documents and Shortfall)
class VendorInline(nested_admin.NestedTabularInline):  # Use Nested Inline
    model = Vendor
    extra = 1
    fields = ('name',)
    inlines = [VendorDocumentInline, ShortfallDocumentInline]  # Documents under Vendors
...
class TenderAdmin(nested_admin.NestedModelAdmin):  # Use NestedModelAdmin
    ...
    inlines = [VendorInline]  # Vendors and their Documents appear inside Tenders

# Register Models
admin.site.register(Department)
admin.site.register(Tender, TenderAdmin)  # Vendors & Docs are inside Tender
